...
std::pair<int, int> DeckState::FindMinStep() {
...
void DeckState::MakeStep(int from, int to) {
    deck_[to].push_back(deck_[from][deck_[from].size() - 1]);
    deck_[from].pop_back();
}

int DeckState::EmptyColumn() {
    for (int i = 0; i < DECK_SIZE; ++i) {
        if (deck_[i].size() == 0)
            return i;
    }
...
std::pair<int, int> DeckState::ExpandColumn() {
    //to be implemented
    return {-1, -1};
}

//убирается 9 последовательных карт из опр колонки
void DeckState::Clean(int column) {
